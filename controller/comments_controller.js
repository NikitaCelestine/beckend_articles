...
class Comments_controller {
    async createComments (req, res){
        ...
        const date = +new Date()
        await comments.create({text_comment: text_comment, date: date, articles_Id: id}).then(()=>{
    ...
    async updateComments (req, res){
        ...
        await comments.update({text_comment: text_comment,  date_update: date_update},  {where: {[Op.and]: [{ articles_Id: id }, { id: comment_Id }]}}).then(()=>{
    ...
    async deleteComments (req, res) { 
        const comment_Id = req.params.comment_id;
        const id = req.params.id
        await comments.destroy({where: {[Op.and]: [{ articles_Id: id }, { id: comment_Id }]}}).then(() => {
